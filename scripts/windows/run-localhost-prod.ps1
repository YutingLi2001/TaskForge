...
$log = Join-Path $logDir "localhost-prod-$ts.log"

Write-Host "Starting PRODUCTION localhost services..."
Write-Host ""
...
& cmd /c "$composeCmd -f $composeFile down >> `"$log`" 2>&1"
...
Write-Host "To reset: run-localhost-prod-reset.bat"
Write-Host "Log:      $log"
Write-Host ""
Read-Host "Press Enter to close"
